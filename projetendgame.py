...
class Naval(tk.Tk):
    ...
    def valider_bateau(self):
        self.bateau.sort()
        self.bateau = self.enlever_doublon(self.bateau)
    ...
    def enlever_doublon(self,liste):
        rep = []
        for elt in liste:
            if elt not in rep:
                rep.append(elt)
    ...
    def definir_Hplateau(self):
        ...
        if len(self.json) == 0:
            ...
            db = mysql.connect(host="localhost",user='root',password='',db='test')
            cursor = db.cursor() 
            cursor.execute(('SELECT compo FROM bataille_navale WHERE compo != %s'),(self.json,))
    ...
    def placer_bateaux(self):
        ...
        db = mysql.connect(host="localhost",user='root',password='',db='test')
        cursor = db.cursor() 
        cursor.execute(('SELECT compo,score FROM bataille_navale'))
        table = cursor.fetchall()
        cursor.close()

        choix = []
        for i in range(len(table)-1):
            scores = json.loads(table[i][1].decode('utf-8'))
            if len(scores) > 0 and sum(scores)/len(scores) > 0:
                choix.append(json.loads(table[i][0].decode('utf-8')))

        compo = choix[rd.randint(0,len(choix)-1)]
        self.positions_bateaux = []
        for coord in compo.values():
            self.positions_bateaux.append(coord)
            for x,y in coord:
                self.plateau[x][y] += 1
    ...
    def choix_intelligent(self):
        ...
        for i in range(len(self.table)):
            data = json.loads(self.table[i][0].decode('utf-8'))
            for bateau in self.positions_bateaux:
                n = len(bateau)
                if n == 3:
                    for x,y in data['30']:
                    ...
                    for x,y in data[str(n)]:
                        stats[x][y] += 1
        
        valeur = [stats[x][y] for x,y in rep]
        moyenne = np.mean(np.array(valeur))
        choix = [index for index,value in enumerate(valeur) if value >= moyenne]
        
        """ FIN """

        return rep[choix[rd.randint(0,len(choix)-1)]]
